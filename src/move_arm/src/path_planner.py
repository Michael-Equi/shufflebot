...
class PathPlanner(object):
    ...
    def plan_to_joint_config(self, joint_config):
        # v = lambda i, vi: dict(zip(right_arm.joint_names(), [0]*(i) + [vi] + [0]*(6-i)))

        print(self._group.get_joints())
        # ['right_arm_mount', 'right_j0', 'right_j1', 'right_j2', 'right_j3', 'right_j4', 'right_j5', 'right_j6', 'right_hand', 'right_gripper_base_joint', 'right_gripper_tip_joint']
        print(type(self._group))
        curr_vals = self._group.get_current_joint_values()
        print(type(self._group))
        self._group.set_joint_value_target(curr_vals)
        self._group.set_start_state_to_current_state()
        plan = self._group.plan()
        return plan

    def plan_to_pose(self, target, constr_dict):
        """
        ...
        if 'orientation_constraints' in constr_dict.keys():
            constraints.orientation_constraints = constr_dict['orientation_constraints']
        if 'joint_constraints' in constr_dict.keys():
            constraints.joint_constraints = constr_dict['joint_constraints']
