...
def rotation_3d(omega, theta):
    ...
    # YOUR CODE HERE
    unit_w_hat = skew_3d(omega) / np.linalg.norm(omega)
    mag_w_theta = np.linalg.norm(omega) * theta
    return np.eye(3) + (unit_w_hat * np.sin(mag_w_theta)) + (np.matmul(unit_w_hat, unit_w_hat) * (1 - np.cos(mag_w_theta)))
...
def hat_3d(xi):
    ...
    xi_hat = np.vstack((skew_3d(omega).T, v.T)).T
...
def homog_3d(xi, theta):
    ...
    if np.linalg.norm(omega) == 0:
        ...
        tra = np.matmul((np.eye(3) - R), np.dot(skew_3d(omega), v)) + omega * np.dot(omega.T, v) * theta
        tra /= np.matmul(omega.T, omega)
...
def prod_exp(xi, theta):
    ...
    for i in range(theta.size):
        g = np.matmul(g, homog_3d(xi_T[i], theta[i]))
